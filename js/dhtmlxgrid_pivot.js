...
dhtmlXGridObject.prototype.makePivot=function(cont,details){
    ...
    if (details.column_list)
   	...
   	var that = this;
	cont.innerHTML="<table style='display:none;' cellspacing='0' cellpadding='0'><tr><td style='width:160px' align='center'></td><td>&nbsp;&nbsp;&nbsp;</td><td></td></tr></table><div></div>";
	...
	this._pUNI=[];
	this._pivotS={ action:(details.action||"0"), value:(typeof details.value != "undefined" ? (details.value||"0") : null), x:(typeof details.x != "undefined" ? (details.x||"0") : null), y:(typeof details.y != "undefined" ? (details.y||"0") : null), ab:(typeof details.ab != "undefined" ? (details.ab||null) : null) };
...
dhtmlXGridObject.prototype._renderPivot2=function(){ 
	...
	for (var i=0; i < x.length; i++) {
		for (var j=0; j < y.length; j++) {
			if (this._pivotS.ab!=null && this._pData[this._pivotS.ab.key]){
				var resA=this._pData[this._pivotS.x];
				var resB=this._pData[this._pivotS.y];
				var resC=this._pData[this._pivotS.value];
				var resD=this._pData[this._pivotS.ab.key];
				val=[];
				for (var k = resA.length - 1; k >= 0; k--){
					if (resA[k]==x[i] && resB[k]==y[j]){
						val.push(resC[k]);
						if (resD && this._pivotS.ab.value){
							for (var l = 0; l < this._pivotS.ab.value.length; l++){
								if (resD[k]==this._pivotS.ab.value[l]){
									this._pgrid.setCellTextStyle(this._pgrid.getRowId(j),i+1,"background-color:"+color);
									break;
								}
							}
						}
				...
				this._pgrid.cells2(j,i+1).setValue(val.join(" | "));
			}
			else{
				this._pgrid.cells2(j,i+1).setValue(action(this._pivotS.x,this._pivotS.y,this._pivotS.value,x[i],y[j],this._pData));
			}
		};
